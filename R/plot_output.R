...
LBSPR_outs <- readRDS(file.path(LBSPR_dir, iter, "LBSPR_results.rds"))
...
Nyears_lf <- length(LBSPR_outs$SPR)
...
Years_lf <- (Nyears_model - Nyears_lf + 1):Nyears_model
	...
	FUN <- function(InputMat, log=TRUE, rel=FALSE){
      ...
      lines(x=which(Years_model %in% Years_lf), y=LBSPR_outs$FM*inp$Data$M, col="red", lwd=2)
      ...
      lines(x=which(Years_model %in% Years_lf), y=rep(1,length(Years_lf)), col="red", lwd=2)
      ...
      lines(x=which(Years_model %in% Years_lf), y=LBSPR_outs$SPR, col="red", lwd=2)
      ...
      if(is.na(F30)==FALSE){
      	points(x=LBSPR_outs$SPR[length(LBSPR_outs$SPR)], y=(LBSPR_outs$FM[length(LBSPR_outs$FM)]*inp$Data$M)/F30, col="red", pch=19, cex=2)
      }
