	...
	for(m in 1:length(dirs)){
		for(i in 1:length(itervec)){
			...
			if(grepl("LBSPR", dirs[m])){
				...
				if(param=="SPR"){
					dev[i,m] <- (rep$SPR[length(rep$SPR)] - true$SPR_alt)
					esterr[i,m] <- log(rep$SPR[length(rep$SPR)]) - log(true$SPR_alt)
					relerr[i,m] <- (rep$SPR[length(rep$SPR)] - true$SPR_alt)/true$SPR_alt
				}
				if(param=="S50"){
