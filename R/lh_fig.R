...
if(save==TRUE) png(file.path(fig_dir, "Life_history_comparison.png"), height=5, width=10, res=200, units="in")
	...
	for(ll in 1:length(lh)){
		...
		if(length(lh)==3){
			if(ll==1) print.letter("Snapper", xy=c(0.75,0.88), font=2, cex=1.3)
			if(ll==2) print.letter("Rabbitfish", xy=c(0.75,0.88), font=2, cex=1.3)
			if(ll==3) print.letter("Hake", xy=c(0.75,0.88), font=2, cex=1.3)
		}
