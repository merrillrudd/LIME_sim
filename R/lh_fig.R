...
if(save==TRUE) png(file.path(fig_dir, "Life_history_comparison.png"), height=5, width=10, res=200, units="in")
	...
	for(ll in 1:length(lh)){
		...
		plot(x=1:length(lh[[ll]]$ages_m), y=lh[[ll]]$Mat_am, type="l", lwd=4, ylim=c(0,1), xaxs="i", yaxs="i", xlim=c(1, length(lh[[ll]]$ages_m)+1), xpd=NA, xlab="", ylab="", xaxt="n", yaxt="n")
		lines(x=1:length(lh[[ll]]$ages_m), y=lh[[ll]]$S_am, lty=3, lwd=4, col="gray", xpd=NA)
		...
		plot(x=1:length(lh[[ll]]$ages_m), y=lh[[ll]]$L_am, lwd=4, type="l", col="blue", xpd=NA, xaxs="i", yaxs="i", ylim=c(0,max(unlist(sapply(1:length(lh), function(xx) lh[[xx]]$L_am)))), xlim=c(1, length(lh[[ll]]$ages_m) + 1), xlab="", ylab="", xaxt="n", yaxt="n")
		...
		if(length(lh)==3){
			...
			axis(1, at=which(lh[[ll]]$ages_m %in% seq(0,max(lh[[ll]]$ages_m)-1,by=1)), labels=seq(0,max(lh[[ll]]$ages_m)-1,by=1), cex.axis=1.2)
		}
	...
	mtext(side=1, "Age (years)", cex=1.5, line=3, outer=TRUE)
