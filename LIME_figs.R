...
plbspr1_e <- median(all_bp$precision[lbspr1_e])

	## improved bias
	(blbspr1_e - blc1_e)/blbspr1_e
	## improved precision
	(plbspr1_e - plc1_e)/plbspr1_e

	## compare life histories -- LIME
	short_lc1 <- lc11[which(grepl("Short", all_dirs[lc11]))]
	all_dirs[short_lc1]
	blc1_short <- median(abs(all_bp$dev[,short_lc1]), na.rm=TRUE)

	med_lc1 <- lc11[which(grepl("Medium", all_dirs[lc11]))]
	all_dirs[med_lc1]
	blc1_med <- median(abs(all_bp$dev[,med_lc1]), na.rm=TRUE)

	long_lc1 <- lc11[which(grepl("Long", all_dirs[lc11]))]
	all_dirs[long_lc1]
	blc1_long <- median(abs(all_bp$dev[,long_lc1]), na.rm=TRUE)
	plc1_long <- median(all_bp$precision[long_lc1])

	## compare life histories -- LBSPR
	short_lbspr1 <- lbspr11[which(grepl("Short", all_dirs[lbspr11]))]
	all_dirs[short_lbspr1]
	blbspr1_short <- median(abs(all_bp$dev[,short_lbspr1]), na.rm=TRUE)

	med_lbspr1 <- lbspr11[which(grepl("Medium", all_dirs[lbspr11]))]
	all_dirs[med_lbspr1]
	blbspr1_med <- median(abs(all_bp$dev[,med_lbspr1]), na.rm=TRUE)

	long_lbspr1 <- lbspr11[which(grepl("Long", all_dirs[lbspr11]))]
	all_dirs[long_lbspr1]
	blbspr1_long <- median(abs(all_bp$dev[,long_lbspr1]), na.rm=TRUE)
	plbspr1_long <- median(all_bp$precision[long_lbspr1])

	## improved bias - short
	(blbspr1_short - blc1_short)/blbspr1_short
	
	## improved bias - medium
	(blbspr1_med - blc1_med)/blbspr1_med

	## better bias using LBSPR - long
	(blc1_long - blbspr1_long)/blc1_long
...
plbspr1_v <- median(all_bp$precision[lbspr1_v])

	## increased bias from equilibrium
...
plbspr1_b <- median(all_bp$precision[lbspr1_b])

	## compare life histories
	short_lbspr1 <- lbspr11[which(grepl("Short", all_dirs[lbspr11]))]
	all_dirs[short_lbspr1]
	blbspr1_short <- median(abs(all_bp$dev[,short_lbspr1]), na.rm=TRUE)
...
## LIME percentage decrease bias from LBSPR for short-lived life history
	(blbspr1_short - blc1_short)/blbspr1_short

## LIME percentage decrease bias from LBSPR for medium-lived life history
	(blbspr1_med - blc1_med)/blbspr1_med

## LBSPR percentage decrease bias from LIME for medium-lived life history
	(blc1_long - blbspr1_long)/blc1_long
	## improved bias
	(blbspr1_b - blc1_b)/blbspr1_b
	## improved precision
	(plbspr1_b - plc1_b)/plbspr1_b
	## increase bias from low Variability
	(blc1_b - blc1_v)/blc1_v
...
plc1_e <- median(all_bp$precision[lc1_e])
	...
	## percentage decrease using index
	(blc1_e - bi1_e)/blc1_e
	(plc1_e - pi1_e)/plc1_e

	## percentage decrease using catch
	(blc1_e - bc1_e)/blc1_e
	(plc1_e - pc1_e)/plc1_e
...
png(file.path(fig_dir, "RelativeError_n1000.png"), height=16, width=42, res=200, units="in")
par(mfrow=c(3,3), mar=c(0,4.5,0,0), omi=c(1.2,1.2,1,1), mgp=c(3,2,0))
## equilibrium
...
## base
i2 <- which(grepl("Short", all_dirs) & grepl("SampleSize_1000/", all_dirs) & grepl("base/", all_dirs) & grepl("Ramp", all_dirs))
# i2 <- i1[-c(which(grepl("LC5/",all_dirs[i1]) | grepl("LC2/",all_dirs[i1])))]
all_dirs[i2]
plot(x=1,y=1,type="n",xaxs="i",yaxs="i",xaxt="n",yaxt="n",xlab="",ylab="",xlim=c(0.5,9.5),ylim=c(-1.5,1))
b1 <- sapply(1:length(i2), function(x) round(median(abs(all_bp$dev[,i2[x]]), na.rm=TRUE),3))
...
i2 <- which(grepl("Medium", all_dirs) & grepl("SampleSize_1000/", all_dirs) & grepl("base/", all_dirs) & grepl("Ramp", all_dirs))
# i2 <- i1[-c(which(grepl("LC5/",all_dirs[i1]) | grepl("LC2/",all_dirs[i1])))]
all_dirs[i2]
plot(x=1,y=1,type="n",xaxs="i",yaxs="i",xaxt="n",yaxt="n",xlab="",ylab="",xlim=c(0.5,9.5),ylim=c(-1.5,2))
b1 <- sapply(1:length(i2), function(x) round(median(abs(all_bp$dev[,i2[x]]), na.rm=TRUE),3))
...
i2 <- which(grepl("Long", all_dirs) & grepl("SampleSize_1000/", all_dirs) & grepl("base/", all_dirs) & grepl("Ramp", all_dirs))
# i2 <- i1[-c(which(grepl("LC5/",all_dirs[i1]) | grepl("LC2/",all_dirs[i1])))]
all_dirs[i2]
plot(x=1,y=1,type="n",xaxs="i",yaxs="i",xaxt="n",yaxt="n",xlab="",ylab="",xlim=c(0.5,9.5),ylim=c(-3,3))
b1 <- sapply(1:length(i2), function(x) round(median(abs(all_bp$dev[,i2[x]]), na.rm=TRUE),3))
...
## base
i2 <- which(grepl("Short", all_dirs) & grepl("SampleSize_1000/", all_dirs) & grepl("base/", all_dirs) & grepl("Increasing", all_dirs))
# i2 <- i1[-c(which(grepl("LC5/",all_dirs[i1]) | grepl("LC2/",all_dirs[i1])))]
all_dirs[i2]
plot(x=1,y=1,type="n",xaxs="i",yaxs="i",xaxt="n",yaxt="n",xlab="",ylab="",xlim=c(0.5,9.5),ylim=c(-1.5,1))
b1 <- sapply(1:length(i2), function(x) round(median(abs(all_bp$dev[,i2[x]]), na.rm=TRUE),3))
...
i2 <- which(grepl("Medium", all_dirs) & grepl("SampleSize_1000/", all_dirs) & grepl("base/", all_dirs) & grepl("Increasing", all_dirs))
# i2 <- i1[-c(which(grepl("LC5/",all_dirs[i1]) | grepl("LC2/",all_dirs[i1])))]
all_dirs[i2]
plot(x=1,y=1,type="n",xaxs="i",yaxs="i",xaxt="n",yaxt="n",xlab="",ylab="",xlim=c(0.5,9.5),ylim=c(-1.5,2))
b1 <- sapply(1:length(i2), function(x) round(median(abs(all_bp$dev[,i2[x]]), na.rm=TRUE),3))
...
i2 <- which(grepl("Long", all_dirs) & grepl("SampleSize_1000/", all_dirs) & grepl("base/", all_dirs) & grepl("Increasing", all_dirs))
# i2 <- i1[-c(which(grepl("LC5/",all_dirs[i1]) | grepl("LC2/",all_dirs[i1])))]
all_dirs[i2]
plot(x=1,y=1,type="n",xaxs="i",yaxs="i",xaxt="n",yaxt="n",xlab="",ylab="",xlim=c(0.5,9.5),ylim=c(-3,3))
b1 <- sapply(1:length(i2), function(x) round(median(abs(all_bp$dev[,i2[x]]), na.rm=TRUE),3))
...
#### compare more years of length comp
...
mtext(side=4, "Base variation", line=2, cex=3)
mtext(side=2, outer=TRUE, line=5, "Estimation error", cex=3)
mtext(side=1, outer=TRUE, line=6, "Years of length composition data", cex=3)
dev.off()
...
axis(1, at=1:7, labels=c("Rich", "Index\nLC10", "Index\nLC1", "Catch\nLC10", "Catch\nLC1", "LC10", "LC1"), cex.axis=2.8)
mtext(side=4, "Base variation", line=2, cex=3)
mtext(side=2, outer=TRUE, line=7, "Proportion of iterations", cex=3)
mtext(side=1, outer=TRUE, line=6, "Data availability scenario", cex=3)
dev.off()
...
abline(h=0, lty=2)
pdf <- cbind.data.frame(sens_bp$esterr[,i1[2]], all_bp$esterr[,i2], sens_bp$esterr[,i1[1]], sens_bp$esterr[,i3[2]], all_bp$esterr[,i4], sens_bp$esterr[,i3[1]])
...
mtext(side=4, "Long-lived", line=2, cex=3)
mtext(side=2, outer=TRUE, line=5, "Estimation error", cex=3)
...
mat <- matrix(c(1,2,3,
				...
				10,11,12,
				10,11,12,
				10,11,12), nrow=10, ncol=3, byrow=TRUE)
nf <- layout(mat)
# layout.show(nf)
col_vec <- c("black", "steelblue", "tomato")
par(mar=c(0,0,0,0), omi=c(1.2,1.2,1,1), mgp=c(3,2,0))
plot(x=lh_list$Short$ages, y=lh_list$Short$S_a, lwd=3, ylim=c(0,1), cex.axis=2.5, yaxt="n", type="o", pch=19, cex=3, xaxs="i", yaxs="i", xpd=NA, xlab="", ylab="")
axis(2, cex.axis=2.5, las=2)
lines(x=lh_list$Short$ages, y=lh_dome_low$Short$S_a, lwd=3, col="steelblue", type="o", pch=19, cex=3, lty=2)
...
plot(x=1,y=1,type="n",xaxs="i",yaxs="i",xaxt="n",yaxt="n",xlab="",ylab="",xlim=c(0.5,3.5),ylim=c(-3,3))
abline(h=0, lty=2)
pdf <- cbind.data.frame(all_bp$esterr[,i2], dome_bp$esterr[,i1[2]], dome_bp$esterr[,i1[1]])
beanplot(pdf, col=lapply(1:length(col_vec), function(x) c(col_vec[x],"black","black","black")), xaxt="n", yaxt="n", xaxs="i", yaxs="i", lwd=3, na.rm=TRUE, what=c(0,1,1,0), beanlines="median", beanlinewd=3, add=TRUE)
allna <- sapply(1:ncol(pdf), function(x) all(is.na(pdf[,x])))
if(any(allna==TRUE)) points(x=which(allna==TRUE), y=rep(0,length(which(allna==TRUE))), pch=4, lwd=4, cex=5)
axis(2,at=seq(-2,3,by=1), las=2,cex.axis=2.5)
...
mat <- matrix(c(1,1,1,2,2,2,3,4,4,4,5,5,5,6,7,7,7,8,8,8,
				9,9,9,10,10,10,11,12,12,12,13,13,13,14,15,15,15,16,16,16,
				17,17,17,18,18,18,19,20,20,20,21,21,21,22,23,23,23,24,24,24), ncol=20, nrow=3, byrow=TRUE)
tt <- layout(mat)
# layout.show(tt)
## fishing mortality
plot(x=1,y=1,type="n",xlim=c(1,20),ylim=c(0,8),xaxs="i",yaxs="i",xlab="",ylab="",xaxt="n",yaxt="n")
print.letter("(a)", cex=2)
axis(2, at=seq(2,8,by=2), las=2, cex.axis=2)
mtext("Fishing mortality", side=2, line=4, cex=2)
mtext("Equilibrium", side=3, line=1, cex=2)
text(x=20, y=8*1.3, "Short-lived", xpd=NA, cex=3.5, col=cols[1])
for(iter in itervec){
	true <- readRDS(file.path(all_plot_vec[1], iter, "True.rds"))
	lines(true$F_t, col=paste0(cols[1],80))
}
plot(x=1,y=1,type="n",xlim=c(1,20),ylim=c(0,8),xaxs="i",yaxs="i",xlab="",ylab="",xaxt="n",yaxt="n")
print.letter("(b)", cex=2)
mtext("Variability", side=3, line=1, cex=2)
for(iter in itervec){
	true <- readRDS(file.path(all_plot_vec[4], iter, "True.rds"))
	lines(true$F_t, col=paste0(cols[1],80))
}
plot(x=1,y=1,type="n",axes=F,ann=F)
...
mtext("Equilibrium", side=3, line=1, cex=2)
text(x=20, y=4*1.3, "Medium-lived", xpd=NA, cex=3.5, col=cols[2])
axis(2, at=seq(1,4,by=1), las=2, cex.axis=2)
for(iter in itervec){
	true <- readRDS(file.path(all_plot_vec[2], iter, "True.rds"))
	lines(true$F_t, col=paste0(cols[2],80))
}
plot(x=1,y=1,type="n",xlim=c(1,20),ylim=c(0,4),xaxs="i",yaxs="i",xlab="",ylab="",xaxt="n",yaxt="n")
print.letter("(d)", cex=2)
mtext("Variability", side=3, line=1, cex=2)
for(iter in itervec){
	true <- readRDS(file.path(all_plot_vec[5], iter, "True.rds"))
	lines(true$F_t, col=paste0(cols[2],80))
}
plot(x=1,y=1,type="n",axes=F,ann=F)
...
mtext("Equilibrium", side=3, line=1, cex=2)
text(x=20, y=0.5*1.3, "Long-lived", xpd=NA, cex=3.5, col=cols[3])
axis(2, at=seq(0.1,0.5,by=0.1), las=2, cex.axis=2)
for(iter in itervec){
	true <- readRDS(file.path(all_plot_vec[3], iter, "True.rds"))
	lines(true$F_t, col=paste0(cols[3],80))
}
plot(x=1,y=1,type="n",xlim=c(1,20),ylim=c(0,0.5),xaxs="i",yaxs="i",xlab="",ylab="",xaxt="n",yaxt="n")
print.letter("(f)", cex=2)
mtext("Variability", side=3, line=1, cex=2)
for(iter in itervec){
	true <- readRDS(file.path(all_plot_vec[6], iter, "True.rds"))
	lines(true$F_t, col=paste0(cols[3],80))
}
## recruitment
plot(x=1,y=1,type="n",xlim=c(1,20),ylim=c(0,4),xaxs="i",yaxs="i",xlab="",ylab="",xaxt="n",yaxt="n")
print.letter("(g)", cex=2)
axis(2, at=seq(1,3,by=1), las=2, cex.axis=2)
mtext("Recruitment", side=2, line=4, cex=2)
for(iter in itervec){
	true <- readRDS(file.path(all_plot_vec[1], iter, "True.rds"))
	lines(true$R_t, col=paste0(cols[1],80))
}
plot(x=1,y=1,type="n",xlim=c(1,20),ylim=c(0,4),xaxs="i",yaxs="i",xlab="",ylab="",xaxt="n",yaxt="n")
...
for(iter in itervec){
	true <- readRDS(file.path(all_plot_vec[2], iter, "True.rds"))
	lines(true$R_t, col=paste0(cols[2],80))
}
plot(x=1,y=1,type="n",xlim=c(1,20),ylim=c(0,4),xaxs="i",yaxs="i",xlab="",ylab="",xaxt="n",yaxt="n")
...
for(iter in itervec){
	true <- readRDS(file.path(all_plot_vec[3], iter, "True.rds"))
	lines(true$R_t, col=paste0(cols[3],80))
}
plot(x=1,y=1,type="n",xlim=c(1,20),ylim=c(0,4),xaxs="i",yaxs="i",xlab="",ylab="",xaxt="n",yaxt="n")
...
## Relative biomass
plot(x=1,y=1,type="n",xlim=c(1,20),ylim=c(0,5),xaxs="i",yaxs="i",xlab="",ylab="",xaxt="n",yaxt="n")
...
for(iter in itervec){
	true <- readRDS(file.path(all_plot_vec[1], iter, "True.rds"))
	lines(true$D_t, col=paste0(cols[1],80))
}
plot(x=1,y=1,type="n",xlim=c(1,20),ylim=c(0,5),xaxs="i",yaxs="i",xlab="",ylab="",xaxt="n",yaxt="n")
...
for(iter in itervec){
	true <- readRDS(file.path(all_plot_vec[2], iter, "True.rds"))
	lines(true$D_t, col=paste0(cols[2],80))
}
plot(x=1,y=1,type="n",xlim=c(1,20),ylim=c(0,5),xaxs="i",yaxs="i",xlab="",ylab="",xaxt="n",yaxt="n")
...
for(iter in itervec){
	true <- readRDS(file.path(all_plot_vec[3], iter, "True.rds"))
	lines(true$D_t, col=paste0(cols[3],80))
}
plot(x=1,y=1,type="n",xlim=c(1,20),ylim=c(0,5),xaxs="i",yaxs="i",xlab="",ylab="",xaxt="n",yaxt="n")
